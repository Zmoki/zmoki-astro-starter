...
Required GitHub secrets for the build step: `PUBLIC_POSTHOG_PROJECT_TOKEN`, `PUBLIC_POSTHOG_HOST`, `PUBLIC_BREVO_ACCOUNT_ID`, `PUBLIC_CLOUDFLARE_TURNSTILE_SITE_KEY`. (The content-image origin is **not** an env var — it's committed as `site.imageOrigin` in `src/site.config.ts`, so CI/preview/production all optimize with no per-environment config.)
...
- **Remote origin** — host originals on a bucket/CDN (e.g. an **R2 bucket on a custom domain**), set **`site.imageOrigin`** in `src/site.config.ts`, and Astro **downloads + optimizes them at build**. Keeps binaries out of git. The origin is pure storage, **decoupled from `site.deploy.platform`**, authorized for optimization via `image.remotePatterns`, and is committed config (not an env var) so it's present in every build environment — including host previews that don't expose env vars. `site.imageOrigin` is also the single source for the CSP `img-src` (no drift).

Config: **`src/image.config.ts`** (`imageCdnHost` + `resolveImageSrc()` — bare key → full URL; no provider/transform logic). Component: **`src/components/Image.astro`** — the one content-image component (wraps `astro:assets`). It emits a schema.org `ImageObject` license JSON-LD for **every** image (from `site.copyright.images.license` + `site.organization`); a **caption** (default slot) additionally wraps it in a `<figure>` + `<figcaption>`. An image `astro:assets` can't optimize (an unauthorized/unknown remote) renders as a plain `<img>` — no bogus repeated-URL srcset, no build-time fetch.

**Authoring** — two ways: `<Image>` (imported asset or remote key/URL; remote needs `width`+`height`; caption ⇒ figure + ImageObject JSON-LD; used for the post cover), and plain Markdown `![]()` (Astro's built-in optimization — local images, and remote URLs whose domain is authorized in `image.remotePatterns`). No custom Markdown-image plugin. Reference remote images by **full URL** in content unless the origin env is always set (a bare key with no base fails the build).
...
**Reachability & caching:** because `site.imageOrigin` is committed, **every** build (CI, host previews, production) optimizes on-origin images, so the origin **must be reachable** at build; an off-origin image (or `imageOrigin: ""`) renders as a plain unoptimized `<img>` with **no fetch** (never fails a build). Astro caches optimized images + fonts in **`node_modules/.astro`** (keyed by content hash) — CI restores it via `actions/cache` (`.github/workflows/ci.yml`) so unchanged images aren't re-processed. The CSP `img-src` host is sourced from the same `site.imageOrigin` (`IMAGE_CDN_HOST` in `src/headers/headers.config.ts`; run `npm run build:headers` after changing it) — single source, no drift.
