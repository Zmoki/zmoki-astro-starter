...
Config: **`src/image.config.ts`** (`imageCdnHost` + `resolveImageSrc()` — bare key → full URL; no provider/transform logic). Component: **`src/components/Image.astro`** — the one content-image component (wraps `astro:assets`). Pass it a **caption** (default slot) and it becomes a `<figure>` + a schema.org `ImageObject` JSON-LD block (Google image-license metadata from `site.copyright.images.license` + `site.organization`); with no caption it's a plain `<img>`.
