...
- **Remote origin** — host originals on a bucket/CDN (e.g. an **R2 bucket on a custom domain**), set `PUBLIC_IMAGE_CDN_HOST`, and Astro **downloads + optimizes them at build**. Keeps binaries out of git. The origin is pure storage, **decoupled from `site.deploy.platform`**, and is authorized for optimization via `image.remotePatterns` (from that env var).
