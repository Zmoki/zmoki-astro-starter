...
- **Content image origin** — optional, and **decoupled from your deploy host**. Set `PUBLIC_IMAGE_CDN_BASE` (e.g. `https://images.example.com`, an R2 bucket on a custom domain) to host image originals externally; Astro downloads + optimizes them at build (keeping binaries out of git). Then add that host to the CSP (`IMAGE_CDN_HOST` in `src/headers/headers.config.ts` → `npm run build:headers`). Leave blank to commit images to `src/images` instead. A build with the origin set must be able to reach it. See the `/images` skill.
