...
Type is **self-hosted via [Astro's Fonts API](https://docs.astro.build/en/guides/fonts/)** (configured in `astro.config.mjs`) and wired into Tailwind's `font-sans` / `font-serif` / `font-mono` families through CSS variables — the site is **all-sans**: Noto Sans for headings and body, Noto Sans Mono for code (`font-serif` stays available but unused). To change them, run **`/brand`** — it walks the full swap (the `fonts` config in `astro.config.mjs`, the Tailwind families, and the brand specimen). Preview at `/-/astro/brand/typography/`.
